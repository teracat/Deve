...
/// </summary>
public abstract class BaseLoggingHandler : DelegatingHandler
{
    ...
    protected BaseLoggingHandler()
        : base(new HttpClientHandler())
    {
    }

    [System.Diagnostics.CodeAnalysis.SuppressMessage("Reliability", "CA2000:Dispose objects before losing scope", Justification = "Disposed in DelegatingHandler")]
    protected BaseLoggingHandler(string outputPrefix)
        : base(new HttpClientHandler())
    {
        _outputPrefix = outputPrefix;
    }

    protected BaseLoggingHandler(HttpMessageHandler innerHandler)
        : base(innerHandler)
    {
    }

    protected BaseLoggingHandler(HttpMessageHandler innerHandler, string outputPrefix)
        : base(innerHandler)
    {
        _outputPrefix = outputPrefix;
    }
