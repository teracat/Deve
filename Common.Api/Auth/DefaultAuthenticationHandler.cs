...
public class DefaultAuthenticationHandler : AuthenticationHandler<AuthenticationSchemeOptions>
{
    ...
    private AuthenticateResult ValidateToken(string scheme, string token, DataOptions options)
    {
        ...
        {
            var principal = UserIdentityMapper.ToClaimsPrincipal(scheme, userIdentity);
    ...
    private static AuthenticateResult GetResultUnauthorized(string langCode) => AuthenticateResult.Fail(ErrorLocalizeFactory.Get().Localize(ResultErrorType.Unauthorized, langCode));
    #endregion
}
