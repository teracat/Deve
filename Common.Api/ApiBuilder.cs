...
namespace Deve.Api
{
    public class ApiBuilder
    {
        ...
        private static ApiBuilder Create(string[] args) => new(args);
        ...
        private ApiBuilder Configure()
        {
            ...
            // Add Cache
            RedisCache? redisCache = AddCache();
        ...
        private WebApplication Build()
        {
            ...
            {
                appAction.Action(webApplication);
            }

            // Add NetCore Log Provider
            Log.Providers.AddNetCore(webApplication.Logger);
        ...
        /// <summary>
        /// Registers the application's data source configuration as a singleton service in the dependency injection container.
        /// </summary>
        /// <remarks>This method retrieves the connection string named "DataSourceConnection" from the
        /// application's configuration and uses it to create a IDataSourceConfig instance. The instance
        ...
        /// null.</param>
        private void AddDataSourceConfig()
        {
            var dsConnectionString = _builder.Configuration.GetConnectionString("DataSourceConnection") ?? string.Empty;
            ...
            _ = _builder.Services.AddSingleton<IDataSourceConfig>(dsConfig);
        }
        ...
        {
            _ = _builder.Services.AddControllers(options =>
            {
        ...
        private void AddAuthentication()
        {
            ...
            var tokenManagerJwt = new TokenManagerJwt(_appSettings.JwtKeys.SigningSecretKey, _appSettings.JwtKeys.EncryptionSecretKey);
            _ = _builder.Services.AddSingleton<ITokenManager>(tokenManagerJwt);
        ...
        private void RegisterCoreServices()
        {
            ...
            _ = _builder.Services.AddSingleton<IHash, HashSha512>();
            ...
            _ = _builder.Services.AddScoped<IDataSource, DataSourceMain>();
            ...
            _ = _builder.Services.AddScoped<IDataOptions, DataOptionsFromContextAccessor>();
            ...
            _ = _builder.Services.AddScoped<IUserIdentityService, ContextAccessorUserIdentityService>();
            ...
            _ = _builder.Services.AddScoped<IAuth, AuthMain>();

            // Registers ShieldMain as the implementation for IShield with singleton lifetime.
            // A single instance is created and shared across the entire application.
            _ = _builder.Services.AddSingleton<IShield, ShieldMain>();
            ...
            _ = _builder.Services.RegisterCoreClasses();
        }
        ...
        private RedisCache? AddCache()
        {
            ...
            if (string.IsNullOrWhiteSpace(redisConnection))
            {
                ...
                _ = _builder.Services.AddSingleton<ICache>(new SimpleInMemoryCache());
        ...
        /// <param name="redisCache">An optional Redis cache instance whose connection will be instrumented for diagnostics if provided.
        /// If null, Redis diagnostics will not be configured.</param>
        private void AddDiagnostics(RedisCache? redisCache)
        {
            ...
            _ = _builder.AddDiagnosticsOpenTelemetry(redisCache?.ConnectionMultiplexer,
