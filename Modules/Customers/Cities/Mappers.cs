...
{
    public static GetList.Query ToQuery(this CityGetListRequest? request) => new(request?.Id, request?.Name, request?.StateId, request?.Offset, request?.Limit, request?.OrderBy);
