    ...
    IRepository<User> repositoryUser) : IGetListQueryHandler<Query, UserResponse>
{
    ...
    private static IQueryable<User> ApplyFilters(IQueryable<User> query, Query request)
    {
    ...
    private static IQueryable<User> ApplyOrder(IQueryable<User> query, Query request, out string orderBy)
    {
        orderBy = request.OrderBy ?? nameof(UserGetListRequest.Name);
