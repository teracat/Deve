...
namespace Deve.Tests
{
    internal class DataSourceMock : Mock<IDataSource>
    {
        public DataSourceMock()
        {
            ...
            Setup(d => d.Users.Update(It.IsAny<User>())).Returns<User>((c) => Update(DataMock.Users, c));
            ...
            Setup(d => d.Countries.Update(It.IsAny<Country>())).Returns<Country>((c) => Update(DataMock.Countries, c));
            ...
            Setup(d => d.States.Update(It.IsAny<State>())).Returns<State>((c) => Update(DataMock.States, c));
            ...
            Setup(d => d.Cities.Update(It.IsAny<City>())).Returns<City>((c) => Update(DataMock.Cities, c));
            ...
            Setup(d => d.Clients.Update(It.IsAny<Client>())).Returns<Client>((c) => Update(DataMock.Clients, c));
            Setup(d => d.Clients.Delete(It.IsAny<long>())).Returns<long>((id) => Delete(DataMock.Clients, id));
            Setup(d => d.Clients.UpdateStatus(It.IsAny<long>(), It.IsAny<ClientStatus>())).Returns<long, ClientStatus>((id,status) => Task.Run(() => id <= 0 ? Utils.ResultError(ResultErrorType.InvalidId) : Utils.ResultOk()));
