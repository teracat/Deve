...
public abstract class TestListDataViewModel : IClassFixture<FixtureMaui>
{
    ...
    internal abstract ListDataViewModel CreateViewModel(INavigationService navigationService, Data.IData data, ISchedulerProvider scheduler);

    [Fact]
    public async Task Initialization_ValidAuthAdmin_HasNoError()
    {
        var schedulerProvider = new TestSchedulers();
        var viewModel = CreateViewModel(_fixture.NavigationService.Object, _fixture.DataAuthAdmin, schedulerProvider);
    ...
    [Fact]
    public async Task Initialization_ValidAuthAdmin_DataNotNull()
    {
        var schedulerProvider = new TestSchedulers();
        var viewModel = CreateViewModel(_fixture.NavigationService.Object, _fixture.DataAuthAdmin, schedulerProvider);
    ...
    [Fact]
    public async Task Initialization_ValidAuthAdmin_DataNotNullAndHasItems()
    {
        var schedulerProvider = new TestSchedulers();
        var viewModel = CreateViewModel(_fixture.NavigationService.Object, _fixture.DataAuthAdmin, schedulerProvider);
    ...
    public async Task SelectedData_NotNull_NavigatesToDetails()
    {
        ...
        var schedulerProvider = new TestSchedulers();
        var viewModel = CreateViewModel(navigationService.Object, _fixture.DataAuthAdmin, schedulerProvider);
        await viewModel.Initialization;
